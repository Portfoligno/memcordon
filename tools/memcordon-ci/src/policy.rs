...
fn check_private_native_diagnostic_job(
    ...
    let command = if candidate {
        "sudo -E ./target/ci/control-bootstrap/ci-bootstrap/memcordon-ci --build-context target/ci/native-inputs.bin suite backend-linux-private-v4 --stage candidate-capability --target native --collector-intent-sha256 \"$COLLECTOR_INTENT_SHA256\" --policy-intent-sha256 \"$POLICY_INTENT_SHA256\" --observer-intent /etc/memcordon/release-trust/candidate-observer.v1.json --observer-intent-sha256 \"$OBSERVER_INTENT_SHA256\""
    } else {
        "sudo -E ./target/ci/control-bootstrap/ci-bootstrap/memcordon-ci --build-context target/ci/native-inputs.bin suite backend-linux-private-v4 --stage final-public --target native --observer-intent /etc/memcordon/release-trust/public-observer.v2.json --observer-intent-sha256 \"$PUBLIC_OBSERVER_INTENT_SHA256\""
    };
    ...
    if candidate {
        ...
        if scalar(environment, "GITHUB_TOKEN") != Some("${{ github.token }}")
            || scalar(environment, "COLLECTOR_INTENT_SHA256")
                != Some("${{ inputs[format('collector_intent_sha256_{0}', matrix.id)] }}")
            || scalar(environment, "POLICY_INTENT_SHA256")
                != Some("${{ inputs[format('policy_intent_sha256_{0}', matrix.id)] }}")
            || scalar(environment, "OBSERVER_INTENT_SHA256")
                != Some("${{ inputs[format('observer_intent_sha256_{0}', matrix.id)] }}")
        {
        ...
        if scalar(invocations[0], "name")
            != Some("Final installed public source suite on the same enrolled host")
        {
        ...
        exact_mapping_keys(
            environment,
            &["GITHUB_TOKEN", "PUBLIC_OBSERVER_INTENT_SHA256"],
            "private final-public suite env",
        )?;
        if scalar(environment, "GITHUB_TOKEN") != Some("${{ github.token }}")
            || scalar(environment, "PUBLIC_OBSERVER_INTENT_SHA256")
                != Some("${{ inputs[format('public_observer_intent_sha256_{0}', matrix.id)] }}")
        {
    ...
    if candidate {
        ...
        let [upload] = uploads.as_slice() else {
            return Err(failure("public raw-only upload count differs"));
        };
        exact_mapping_keys(
            upload,
            &["name", "if", "uses", "with"],
            "public raw-only upload",
        )?;
        let with = mapping(
            upload
                .get(key("with"))
                .ok_or_else(|| failure("public raw-only upload inputs absent"))?,
            "public raw-only upload inputs",
        )?;
        exact_mapping_keys(
            with,
            &[
        ...
        if scalar(upload, "name")
            != Some("Retain origin-backed public raw evidence only after exact all25 replay")
            || scalar(upload, "if") != Some("success()")
            || scalar(with, "name") != Some("release-private-public-raw-${{ matrix.id }}")
            || scalar(with, "path")
                != Some("target/ci/reports/private-public-raw-v1/linux-${{ matrix.id }}")
            || scalar(with, "if-no-files-found") != Some("error")
            || with.get(key("retention-days")).and_then(Value::as_u64) != Some(14)
            || with.get(key("compression-level")).and_then(Value::as_u64) != Some(0)
        {
            return Err(failure(
                "public producer upload must contain raw evidence only, never P/CP",
            ));
        }
    }
    Ok(())
