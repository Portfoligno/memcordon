...
fuzz_target!(|data: &[u8]| {
    if let Ok(input) = std::str::from_utf8(data) {
        // Both raw bytes and an optional newline-terminated seed record use the same parser.
        for identity in [Some(input), input.strip_suffix('\n')]
            .into_iter()
            .flatten()
        {
            if let Ok(inode) = parse_namespace_identity(identity, "pid") {
                assert_eq!(
