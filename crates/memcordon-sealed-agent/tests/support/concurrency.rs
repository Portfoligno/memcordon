...
use memcordon_sealed_agent::linux::launch::{self, TargetExecStatus};
...
fn run_worker(directory: &Path) {
    ...
    let started_monotonic_millis = memcordon_sealed_agent::linux::clock::monotonic_millis()
        .expect("worker start time must be available");
    ...
    let terminal_monotonic_millis = memcordon_sealed_agent::linux::clock::monotonic_millis()
        .expect("worker terminal time must be available");
    ...
    let record_absent = !Path::new(memcordon_sealed_agent::linux::STATE_ROOT)
        .join(&identity)
        .exists();
    let cgroup_absent = !Path::new(memcordon_sealed_agent::linux::CGROUP_ROOT)
        .join(&identity)
        .exists();
...
fn wait_for_overlap(
    ...
    loop {
        ...
        let records_live = ready.iter().all(|item| {
            Path::new(memcordon_sealed_agent::linux::STATE_ROOT)
                .join(&item.identity)
                .is_file()
        });
...
fn validate_results(
    ...
    for ((ready, result), members) in ready.iter().zip(results).zip(live_members) {
        ...
        assert!(
            !Path::new(memcordon_sealed_agent::linux::CGROUP_ROOT)
                .join(&result.identity)
                .exists()
        );
