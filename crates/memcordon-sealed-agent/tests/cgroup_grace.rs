...
fn command_exit_grace_returns_when_cgroup_drains() {
    ...
    let deadline_exceeded =
        memcordon_sealed_agent::linux::launch::wait_command_exit_grace_for_test(
            &cgroup,
...
fn attempt_deadline_remains_authoritative_during_command_exit_grace() {
    ...
    let deadline = memcordon_sealed_agent::linux::clock::monotonic_millis()
        .unwrap()
        .saturating_add(75);
    let started = Instant::now();
    let deadline_exceeded =
