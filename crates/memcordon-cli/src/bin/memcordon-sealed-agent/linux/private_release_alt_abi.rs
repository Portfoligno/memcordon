...
fn run_branch(
    ...
    verify_child_proc(pid, uid, gid, inherited_filter_count)?;
    parent
        .write_all(&[GO])
        .map_err(|error| format!("MCSEALED-PRIVATE-RELEASE: x32 release: {error}"))?;
    let native_response = if branch == Branch::Native {
        let mut response = [0_u8; 32];
        parent
            .read_exact(&mut response)
            .map_err(|error| format!("MCSEALED-PRIVATE-RELEASE: native getpid: {error}"))?;
        let expected = native_response_digest(&challenge, filter, pid, pid as libc::c_long);
        if response != *expected.bytes() {
            return Err("MCSEALED-PRIVATE-RELEASE: native getpid response differs".into());
        }
        Some(expected)
    } else {
        None
...
fn child_branch(
    ...
    if installed.instruction_digest != *filter.bytes() {
        return Err("MCSEALED-PRIVATE-RELEASE: x32 installed filter differs".into());
    }
