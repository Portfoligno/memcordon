...
fn peer_authorization_requires_root_or_exact_access_group() {
    let authorized = crate::linux::service::peer_is_authorized_for_test;
...
fn direct_cgroup_membership_short_circuits_inventory() {
    ...
    assert!(
        crate::linux::service::cgroup_membership_is_sealed("0::/\n").is_ok_and(|sealed| !sealed)
    );
    assert!(crate::linux::service::cgroup_membership_is_sealed("0::/").is_err());
    assert!(
        crate::linux::service::cgroup_membership_is_sealed(&format!(
            "0::/{}\n",
            "x".repeat(70 * 1024)
        ))
        .is_err()
