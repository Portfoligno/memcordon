...
fn private_receipt_uses_non_root_cgroup_controls_and_memory_readback() {
    ...
    crate::linux::cgroup::prepare_private_root_for_test(temporary.path()).unwrap();
}
...
fn private_root_without_non_root_kill_or_memory_controller_fails_closed() {
    ...
    let error = crate::linux::cgroup::prepare_private_root_for_test(temporary.path()).unwrap_err();
...
fn attempt_control_inventory_requires_memory_and_retirement_interfaces() {
    ...
    crate::linux::cgroup::verify_attempt_controls_for_test(temporary.path()).unwrap();

    std::fs::remove_file(temporary.path().join("memory.swap.max")).unwrap();
    let error =
        crate::linux::cgroup::verify_attempt_controls_for_test(temporary.path()).unwrap_err();
