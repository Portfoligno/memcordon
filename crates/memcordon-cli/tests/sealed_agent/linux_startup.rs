...
use crate::linux::startup::StartupPhase;
...
fn startup_failure_is_typed_bounded_and_atomically_clearable() {
    ...
    crate::linux::startup::record_for_test(&path, StartupPhase::Qualification, &detail).unwrap();
    let record = crate::linux::startup::read_for_test(&path)
        .unwrap()
        .unwrap();
    ...
    crate::linux::startup::clear_for_test(&path).unwrap();
    assert!(
        crate::linux::startup::read_for_test(&path)
            .unwrap()
            .is_none()
    );
}
...
fn startup_failure_rejects_unknown_fields_and_unstable_codes() {
    let temporary = TempDir::new().unwrap();
    let path = temporary.path().join("startup.json");
    let error = crate::linux::startup::record_for_test(
        &path,
    ...
    let error = crate::linux::startup::read_for_test(&path).unwrap_err();
